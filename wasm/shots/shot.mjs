...
//                                                           do not write)
//   node shot.mjs --dir www --claims claims.json --out …   (refuse a PNG in
//                                                           which a claimed
//                                                           string is clipped)
//
...
usage: node shot.mjs --dir <served dir> [--claims <json>] --out <file|-> <script.js>
  ...
  --claims  internal/shotclaims as JSON (go run ./claims): the strings each
          PNG claims, checked whole inside the frame before it is written`);
    process.exit(2);
}

function parseArgs(argv) {
    const out = { dir: "", out: "", script: "", claims: "" };
...
async function main() {
    ...
    try {
        ...
        // Before the shutter, not after: a picture that fails its claims is
        // not written, so docs/images never holds one. Keyed by the output's
        // file name, which is what the manifest is keyed by; a shot with no
        // entry (a composite, or a file taken under another name) is not
        // checked.
        if (args.claims) {
            const manifest = JSON.parse(readFileSync(resolve(args.claims), "utf8"));
            const shows = manifest[basename(args.out)];
            if (shows) {
                const clipped = await evaluate(
                    `(${CLIPPED})(${JSON.stringify(shows)}, ${JSON.stringify(clip)})`);
                if (clipped.length) {
                    throw new Error(`${basename(args.out)} was not written: ` +
                        `internal/shotclaims says it shows text the frame ` +
                        `does not show whole —\n  ${clipped.join("\n  ")}\n` +
                        `Move the frame (the script's scroll or size) or ` +
                        `claim what is in it.`);
                }
            }
        }
