...
| `inert-follows-focus` | An `AccessibilitySelectionFollowsFocus` on a node whose role is not one of `core.KeyboardComposites()` — a `listbox`, a `tablist` or a `toolbar` | The flag says what a composite's **arrow keys** do, and only those three roles have any. The WASM runtime writes `data-grmob-selection-follows-focus` for any node that asks — deliberately, since consulting the composite tables where attributes are written would put those tables in two places — so the claim reaches the DOM and nothing ever reads it back; `htmlout` and both natives write nothing at all. The commonest way to get here is no role: the strip looks right, announces as a plain box, and the flag is the only evidence anyone meant a composite |
| `nested-composite` | A container with a keyboard pattern inside another one — a `tablist` in a `toolbar`, a `listbox` in a `listbox` | Both keep their own roving `tabindex`, so the pair is **two tab stops**. What the outer widget's arrows then do depends on the pair, and the finding says which — see below. ARIA describes one stop: the pattern makes the inner widget's current member the outer widget's member, which means two widgets writing `tabindex` onto one element and needs an owner rule this framework does not have. Every control stays reachable either way, so this is a divergence worth knowing you built rather than a break |
| `unusable-value-range` | An `AccessibilityValue` whose numbers no platform can use as written — a position or a bound that is not a number, or a range whose `Max` is at or below its `Min` | Every target resolves it and no two of them resolve it the same way. A browser reads an unparseable `aria-value*` as `0` and pins the bar there; Compose reads it as absent and announces an **indeterminate** bar; `core.Progress` agrees with Compose; SwiftUI has no numeric value property at all. An empty range is worse still — Compose's `ProgressBarRangeInfo` throws on one, so `Renderer.kt` drops the property rather than crash a render over an annotation. Nothing errors and the bar looks right everywhere, because the fill on screen is drawn from the caller's own float and never went through this vocabulary. The commonest cause is a formatting bug — `"45%"`, or a byte count in scientific notation |
...
| `partial-sort` | A `comps.DataTable` sorting client-side (its active `Sort` names a column with a `Less`) while its `Pagination` declares a `PageCount` — the caller saying the server chose the rows | The table can only order the window it holds, so a header claiming an ordering over the table delivers one over a page of it. A partial sort looks exactly like a working sort; the rows that disprove it are the ones not fetched. Set `Sortable` without `Less` and put the sort in the query |

### The two reaches of `nested-composite`

Two tab stops is what every nested pair has in common. What differs is whether
the outer widget's member walk **stops** at the inner one or **descends through**
it, and the two are different bugs:

| the outer container | what its arrows do | why |
|---|---|---|
| a `toolbar`, around anything | step over the inner widget whole | ARIA defines no `toolbaritem`, so nothing says whose a plain button inside a nested widget is |
| a `listbox` in a `listbox`, a `tablist` in a `tablist` | step over it | the two would otherwise pool their members, and one widget's arrows would walk out into the other's rows |
| a `listbox` around a `tablist`, a `tablist` around a `listbox`, either around a `toolbar` | **land inside it** | an `option` below a `tablist` is still the listbox's option — the roles say whose it is — so the walk carries on, and any member of the outer's role buried in the inner's subtree joins the outer's rotation |

The rule is `core.CompositeWalkStopsAt`, and it is one statement read by the
audit's sentence and by the runtime's two member walks at once. The report used
to say "steps over" for every pair, which was right for four of the nine and
would have sent an author looking in the wrong place for the other five.
