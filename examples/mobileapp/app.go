...
//
// The transport is comps.AudioPlayer, which this tab used to build by hand
// (a scrub state, a between-justified time row, the rate cycle and an m:ss
// formatter); the widget is that code, extracted. What stays here is the
// track and the raw state line, which is diagnostics for this demo rather
// than something a player screen shows.
func audioTab() core.View {
	return core.ComponentFunc(func(ctx *core.Context) *core.Node {
		status := hooks.UseAudio(ctx)

		line := "Nothing loaded"
		if status.Track.URL == demoTrack.URL {
		...
		return core.Column(
			...
			comps.AudioPlayer{
				Track: demoTrack,
				// 1 → 1.25 → 1.5 → 2 → 0.75 → 1, the cycle the hand-built
				// speed button had.
				Rates:    []float64{1, 1.25, 1.5, 2, 0.75},
				ShowStop: true,
			},
			core.Text(line, core.UseStyle(core.Style{FontSize: 13, TextColor: "#3C3C4399"})),
		).Render(ctx)
	})
}
