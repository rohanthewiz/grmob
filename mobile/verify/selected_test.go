...
func TestBothNativesSpeakTheCurrentDateIntoTheName(t *testing.T) {
	for _, pin := range []struct{ file, expr, why string }{
		...
		{swiftStyle, `kind == "date" ? label + ", " + grMobTodayWord() : label`,
			"the suffix itself (the label, because a value does not reach a container cell; see its doc)"},
		...
		{kotlinStyle, `fun grMobCurrentLabel(label: String, kind: String, valueText: String = ""): String =`,
			"the helper that folds the current date into the contentDescription when the state slot is taken"},
		{kotlinStyle, `if (kind == "date" && label.isNotEmpty() && valueText.isNotEmpty()) "$label, ${grMobTodayWord()}" else label`,
			"the fallback suffix, and no bare suffix on a node with no name"},
		{kotlinStyle, `if (kind == "date" && label.isNotEmpty() && valueText.isEmpty()) grMobTodayWord() else ""`,
			"the word as the stateDescription, its first home"},
		{kotlinStyle, `if (currentState.isNotEmpty()) stateDescription = currentState`,
			"boxModifier spending the word in the semantics block"},
		...
		{kotlinStyle, `listOf(grMobCurrentLabel(accessibilityLabel, currentKind, valueRange.text), accessibilityHint)`,
