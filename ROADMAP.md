...
- [x] Foldables and window size — `core.CurrentWindow` / `core.OnWindow` /
      `hooks.UseWindow` over the `"window"` host event: the window's size in
      layout units, Material's size classes, and the fold (flat / half-opened,
      vertical / horizontal, separating, occluding, bounds) with tabletop and
      book postures derived from it. Jetpack WindowManager on Android, the
      root view's size on iOS (no fold), viewport segments plus device
      posture in the browser. `comps.TwoPane` splits at a separating hinge,
      by ratio from medium width, and stacks or picks one pane when compact;
