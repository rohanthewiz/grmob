...
func Cases() []Case {
	...
	return []Case{
		...
		{"fit, wide viewBox in a square box", 100, 50, 300, 300, false, wire(core.Line(0, 0, 100, 50))},
		// And on the horizontal axis.
		{"fit, tall viewBox in a wide box", 10, 40, 200, 100, false, wire(core.Rect(0, 0, 10, 40))},
		// Stretch: two scales, no offset.
		{"stretch", 100, 50, 300, 300, true, wire(core.Polyline(0, 50, 50, 0, 100, 25))},
		// Every opcode, cubics included, through a non-trivial mapping.
		{"a circle, every opcode", 100, 50, 320, 120, false, wire(circle)},
		// A hand-assembled node's zero viewBox reads as 100, not a division by zero.
		{"zero viewBox reads as 100", 0, 0, 50, 50, false, []float64{0, 100, 100}},
		// Truncation and an unknown opcode end the path; neither fails it.
		{"truncated operation", 100, 100, 100, 100, true, []float64{0, 1, 2, 1, 3}},
		{"unknown opcode", 100, 100, 100, 100, true, []float64{0, 1, 2, 9, 1, 3, 4}},
	}
}
